...
st.set_page_config(page_title="Hedge Fund AI: Proportional Allocation", layout="wide", page_icon="🏦")
st.title("🧠 Hedge Fund AI: Proportional Allocation (Risk Dağıtımlı)")
...
with st.sidebar:
    ...
    st.success("✅ Exhaustive Search Aktif: 1'den 10'a kadar tüm öğrenme seviyeleri taranıyor.")
    st.info("Strateji: ROI'ye göre Orantılı Dağıtım (Proportional Allocation).")
...
# --- ORTAK ANALİZ FONKSİYONLARI (bot.py ile aynı) ---
        ...
        for i, (idx, row) in enumerate(updated.iterrows()):
            ...
            if res:
                ...
                with st.expander(f"📊 {ticker} | ROI: %{res['roi']:.2f} | Gen: {res['gen_count']}"):
                    ...
                    with tab2:
                        # HATA DÜZELTİLDİ: Sözlükten güvenli DataFrame oluşturma
                        w = info.get('weights', {})
                        if w:
