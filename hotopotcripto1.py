...
<style>
    ...
    .header-box {
        ...
        padding: 25px; border-radius: 12px; border-left: 5px solid #00d2ff;
    ...
    .spec-card {
        background-color: #1c1e21; padding: 15px; border-radius: 10px;
        border: 1px solid #333; margin-bottom: 10px; font-size: 13px;
    }
</style>
<div class="header-box">
    <div class="header-title">🧠 Hedge Fund AI: V8 (AutoML & Tournament)</div>
    <div class="header-sub">Auto-Imputation Selection • XGBoost Hyperparameter Tuning • Ensemble Transparency</div>
</div>
...
class Brain:
    ...
    def run_tournament(self, df):
        ...
        roi = (sim_solo if winner == "Solo Optimized XGB" else sim_ens) - 100
        
        return {
            'prob': prob, 'winner': winner, 'roi': roi, 'xgb_params': xgb_params,
            'eq_ens': eq_ens, 'eq_solo': eq_solo, 'dates': test.index
        }
...
tab1, tab2, tab3 = st.tabs(["📊 Bot Kontrol & AutoML", "📑 Canlı Veri", "📜 Geçmiş Kayıtları"])
    ...
    with tab1:
        ...
        if st.button("🚀 AUTOML BAŞLAT (Impute & Tune)", type="primary"):
                ...
                if df is not None:
                    ...
                    if df_clean is not None:
                        ...
                        with st.expander(f"{ticker} | {decision} | {res['winner']} (ROI: %{res['roi']:.1f})"):
                            ...
                            c1.markdown(f"**Temizlik Kazananı:** `{imp_winner}`")
                            c1.caption(f"Hata (RMSE): {imp_scores[imp_winner]:.5f}")
                            
                            c2.markdown(f"**XGBoost Parametre:** `{res['xgb_params']}`")
                            c2.caption("Auto-Tuning ile seçildi.")
                            
                            c3.markdown("**Ensemble İçeriği:**")
                            c3.caption(f"RF(d=5) + ETC(d=5) + XGB({res['xgb_params']}) + HMM")
            ...
            if buy_orders and pool_cash > 5:
                ...
                updated_pf.at[updated_pf.index[0], 'Nakit_Bakiye_USD'] += pool_cash
