...
def load_portfolio(sheet):
    ...
    if 'Bot_Durum' not in df.columns: df['Bot_Durum'] = "Beklemede" # YENİ: İlerleme durumu
...
class GrandLeagueBrain:
    ...
    def run_league(self, df):
        impute_methods = ['Baydo', 'Linear'] # Hız için kısalttım
        strategies = []
...
if __name__ == "__main__":
    logger.info("Bot Started V16 (Status Monitor).")
    ...
    while True:
        try:
            ...
            if not pf.empty and 'Bot_Trigger' in pf.columns:
                if str(pf.iloc[0]['Bot_Trigger']).upper() == 'TRUE': trigger = True
            
            elapsed = (datetime.now() - last_run).total_seconds() / 60
            if trigger or elapsed >= LOOP_INTERVAL_MINUTES:
                logger.info("--- Çalışıyor ---")
                
                # 1. HEMEN DURUM GÜNCELLE (Kullanıcı görsün)
                now_str = str(datetime.now(pytz.timezone('Turkey')).strftime('%Y-%m-%d %H:%M'))
                pf['Bot_Durum'] = "⏳ İşleniyor..."
                ...
                with ThreadPoolExecutor(max_workers=5) as executor:
                    ...
                    for i, future in enumerate(as_completed(futures)):
                        # İlerleme güncelle (Her 2 coinde bir kaydet)
                        if i % 2 == 0:
                            updated['Bot_Durum'] = f"Analiz: {i}/{len(updated)}"
                ...
                updated['Bot_Durum'] = "✅ Hazır" # İşlem bitti
                save_portfolio(updated, pf_sheet)
                last_run = datetime.now()
                logger.info("Tur Bitti.")
                
        except Exception as e: logger.error(f"Hata: {e}")
        time.sleep(15) # 15 saniyede bir kontrol (Hızlı tepki için)
